...
if (!empty($_POST['lastName']) && !empty($_POST['firstName']) && !empty($_POST['birthDate'])) {
    ...
    $newCustomer = $db->prepare('UPDATE `clients` SET `lastName` = :lastName, `firstName` = :firstName, `birthDate` = :birthDate WHERE `id`=22;');
...
// On lance une commande SQL pour récupérer toutes les données de la table clients où id = 22
$customers = $db->query('SELECT `lastName`, `firstName`, `birthDate`, `card`, `cardNumber` FROM `clients` WHERE `id` = 22;');
$customer = $customers->fetch(PDO::FETCH_OBJ);
// Pour chaque client dans la table on récupère les infos et on les affiches
?>
<h1>Exercice 4</h1>
<div class="container-fluid">
    <form class="form-horizontal" action="index.php?tab=4" method="POST">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="checkbox" <?php echo ($customer->card == 1) ? 'checked' : ''; ?> name="fidelityCard">
            </div>
        </div>
        <div class="form-group">
