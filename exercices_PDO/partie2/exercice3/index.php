...
if (!empty($_POST['showTitle']) && !empty($_POST['showArtist']) && !empty($_POST['showDate']) && isset($_POST['showType']) && isset($_POST['showKind1']) && isset($_POST['showKind2']) && !empty($_POST['showDuration']) && !empty($_POST['showStartTime'])) {
    ...
    $newShow = $db->prepare('INSERT INTO `shows` (`title`, `performer`, `date`, `showTypesId`, `firstGenresId`, `secondGenreId`, `duration`, `startTime`) VALUES (:title, :performer, :date, :showTypesId, :firstGenresId, :secondGenreId, :duration, :startTime);');
...
<div class="container-fluid">
    <form class="form-horizontal" action="index.php?tab=3" method="POST">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="text" class="form-control" name="showArtist">
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-sm-2" for="showDate">Date du spectacle:</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" placeholder="yyyy-mm-dd" name="showDate">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select class="form-control" name="showKind1">
                    ...
                    foreach ($genre as $genreInfo) {
                        ?>
                        <option value="<?= $genreInfo->id ?>"><?= $genreInfo->genre ?></option>
                        <?php
                    }
                    ?>
                </select>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="text" class="form-control" placeholder="hh:mm:ss" name="showDuration">
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-sm-2" for="showStartTime">Heure de début:</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" placeholder="hh:mm:ss" name="showStartTime">
            </div>
        </div>
        <div class="form-group">
